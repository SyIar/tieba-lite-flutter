...
| Followed, pinned, recent forums; list/grid | `HomePage`, account-scoped pins and recents; recent forums appear before followed forums; followed rows show level and check-in status | Recents retain five distinct forums in FIFO order, newest arrivals displayed first; revisits update metadata without changing queue position. Missing member counts are not displayed as zero. Full forum history remains separate. Followed forums need an authenticated account |
...
| Forum information, following, sign-in, ordinary/featured lists and sort | Native forum header, API actions, digest/sort filters, paginated cards; successful sign-in immediately disables the completed button | Guest forum list/detail/rules API probed; Flutter fixtures cover nested sign state, request success/failure, reload failure and return-to-home refresh; authenticated device acceptance pending |
| Pinned forum threads | One compact list of tappable, single-line titles above ordinary thread cards; no author, excerpt, media or counters; scrolls with the page | Deduplicated by thread ID across pages; refresh replaces the pin list; unpinned threads return to ordinary cards; block/video filters retained. Pagination and filtering covered by a Flutter fixture |
